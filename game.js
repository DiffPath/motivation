...
class AdventureEngine {
    ...
    update(dt) {
        ...
        if (window.gridEntities && window.gridEntities[coordKey] && !inEncounter && this.spawnImmunityTimer <= 0) {
            ...
            // Safely ignore chests we've already looted
            if (!entityType.endsWith('_opened')) {
...
// We'll calculate what the visibility SHOULD be right now based on distance
                ...
                // --- 2. CHEST RENDERING ---
                let isOpened = entityKey.endsWith('_opened');
                let baseChestKey = isOpened ? entityKey.replace('_opened', '') : entityKey;
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (activityForm) {
        activityForm.addEventListener('submit', function (e) {
            ...
            // --- COMPUTE ENERGY GAIN (scaled by INT stat) ---
            const intStat = getTotalStat(data, 'int') || 5;
            const pendingEnergy = Math.floor(12 + (intStat * 2.5));

            // --- STORE CHRONICLE (activity always recorded immediately) ---
            ...
            if (existingIndex !== -1) {
                ...
                data.eternal_chronicles.unshift({ date: logDate, stats: todaysDeeds });
...
<div class="placeholder-card" style="padding: 24px; border-color: var(--accent-color); position: relative;">
    ...
    function checkAndApplyPendingRewards() {
        ...
        showDailyRewardModal(rewardDates, totalXP, totalEnergy, leveledUp, levelsGained, data.stats.level);
    }
    ...
    function showDailyRewardModal(dates, xp, energy, leveledUp, levelsGained, newLevel) {
    ...
    window.resolveEncounter = async function (x, y, action, oldPixelX, oldPixelY) { // Added async
        ...
        if (action.startsWith('fight_player_')) {
            ...
            setTimeout(() => {
                ...
                let logMsg = `✨ You opened the ${config.name} and found treasure!`;
                ...
                db.ref(entityPath).set(chestType + '_opened');
                if (window.gridEntities) window.gridEntities[key] = chestType + '_opened'; // <-- Instantly updates the engine!
                updateCharacterTabUI();
            }, 1800);
    ...
    function applyZoom(newSize) {
        // UPDATED: Enforce the new boundaries (40 to 120)
        newSize = Math.max(20, Math.min(160, newSize));
    ...
    window.renderFriendCharts = function (friendName) {
        ...
        if (ctxTotal) {
            ...
            window[`fchartInst_${fn}_total`] = new Chart(ctxTotal, {
                ...
                data: {
                    labels: totalLabels,
                    datasets: [{ label: 'Total XP', data: totalData,
        ...
        if (ctxDaily) {
            ...
            window[`fchartInst_${fn}_daily`] = new Chart(ctxDaily, {
                ...
                data: {
                    labels: dailyLabels,
                    datasets: [{ label: 'Daily XP', data: dailyData,
